                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        @foreach ($pagos_pendientes as $p)
                            <tr>
                                <td>
                                    <a href="{{ route('reservations.show', $p->FOLIO_RESERVACION) }}">{{ $p->FOLIO_RESERVACION }}</a>
                                </td>
                                <td>
                                    <a href="{{ route('users.show', $p->ID_CLIENTE) }}">
                                        {{ $p->NOMBRE_CLIENTE }}
                                    </a>
                                </td>
                                <td>
                                    @php
                                         switch ($p->ESTATUS_RESERVACION) {
                                            case 'Autorizada':
                                                $color = '#3ebef3';
                                                break;
                                            case 'Cancelada':
                                    ...
                                    @endphp 
                                    <label for="" class="label" style="background-color: {{ $color }}">
                                        {{ $p->ESTATUS_RESERVACION }}
                                    </label>
                                </td>
                                <td>{{ $p->HOTEL }}</td>
                                <td>{{ $p->FECHA_LIQUIDAR_HOTEL }}</td>
                                <td>${{ $p->TARIFA_HOTEL }}</td>
                                <td>{{ $p->DESTINO }}</td>
                            </tr>
                        @endforeach
                    </tbody>
...
<div class="row">
    ...
    <div class="col-md-6">
        <div class="card">
            ...
            <div class="card-body table-responsive">
                <table class="table table-hover" id="mis_asignaciones" style="width:100%">
                    <thead>
                        <tr>
                            <th>
                                Folio
...
<script>
    $(document).ready(function() {
        ...
        var asignadas = $('#mis_asignaciones').DataTable({
            processing: true,
            serverSide: true,
            bDestroy: true,
            bInfo: false,
            searching: false,
            ajax: baseadmin + "mis-reservaciones-asignadas",
            columns: [
                {data: 'id', name: 'id'},
