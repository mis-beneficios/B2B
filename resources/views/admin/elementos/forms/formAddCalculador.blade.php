                ...
                <div class="form-row">
                    <div class="form-group col-lg-4 col-md-12">
                        ...
                        <select class="form-control" id="metodo_pago" name="metodo_pago">
                            ...
                            @foreach ($tipo_pago_g as $tipo => $key)
                            <option value="{{ $tipo }}" @if ($con->tipo_pago == $tipo)
                            selected 
                            @endif> {{ $key }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-lg-3 col-md-6">
                        ...
                        <input autocomplete="off" class="form-control fecha_primer_descuento datepicker"  name="fecha_primer_descuento" type="text" value="{{ $data_pagos['fecha_inicial'] }}"/>
                    </div>
...
{{ ($data_pagos['total_segmentos'] != 0) ?  $data_pagos['num_segmentos_pendientes'] : $con->estancia->cuotas }}
                        ...
                        <input class="form-control-range mt-2" id="num_segmentos" max="100" min="1" name="num_segmentos" oninput="$('#rangeval').html($(this).val())" type="range" value="{{ ($data_pagos['total_segmentos'] != 0) ?  $data_pagos['num_segmentos_pendientes'] : $con->estancia->cuotas }}"/>
                        ...
                        <input type="hidden" value="{{ $data_pagos['num_segmentos_pendientes'] }}" name="pagos_pendientes">
                    ...
                    <div class="form-group col-lg-2 col-md-6 justify-content-center align-content-between">
                        <div class="btn-group" data-toggle="buttons">
                            <label class="btn btn-info btn-sm mt-4 @if ($con->aplica_descuento == 1) active @endif">
                                <input type="checkbox" id="descuento" value="{{ $con->estancia->descuento }}" data-descuento="{{ $con->estancia->descuento }}"  name="descuento" @if ($con->aplica_descuento == 1) checked @endif>
                                    Aplicar descuento
                                </input>
                            </label>
                        </div>
                    ...
                    <div class="col-md-12 mr-0">
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                Precio: <strong id="precio_compra" class="text-danger"> ${{ number_format($con->precio_de_compra,2,'.','') }}</strong>
                            </div>
                            <div class="col-md-3">
                                Pagado: <strong id="pagado" class="text-danger">${{ number_format($data_pagos['cantidad_pagada'],2,'.','') }}</strong>
                            </div>   
                            <div class="col-md-3">
                                Pendiente: <strong id="prendiente_pago" class="text-danger">${{ number_format($data_pagos['cantidad_pendiente'],2,'.','') }}</strong>
                            </div>   
                        </div>
