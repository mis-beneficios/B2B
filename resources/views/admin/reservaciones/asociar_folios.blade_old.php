<div class="card-body">
    @if (count($reservacion->contratos) != 0)
            ...
            @php
                ...
                $nochesQuedan = 0;
            @endphp
    <form action="{{ $route}}" class=" m-t-10" id="form_reservaciones_edit" method="PUT">
        @csrf
        <input id="reservacion_id" name="reservacion_id" type="hidden" value="{{ $reservacion->id }}"/>
        <h2>Reservacion #{{$reservacion->id}}</h2>
        <div class="row">
                ...
                <table class="table table-condensed table-hover table-stats" style="font-size: 12px;">
                    ...
                    @if($vinculado->id == $contrato->id)
                        @php
                            ...
                            $contratoVinculado = $vinculado->contrato_id;
                            $reservacionVinculada =  $vinculado->reservacione_id;
                        @endphp
                        @endif
                    @endforeach
                    @php
                        // $class = ($contratoVinculado > 0) ? 'table-info' : (count($contrato->reservacion) === 0 ? '' : 'table-dark');
                        $class = ($contratoVinculado > 0) ? 'table-info' : ($contrato->estatus == 'comprado' ? '' : 'table-dark');
                    @endphp
                    <tr class="{{ $class }}">
                        <td >
                            {{ $contrato->id }}
                        ...
                        <td>
                            <div class="input-group">
                                <span class="input-group-addon">
                                    {{-- Codigo correcto funcionamiento --}}
                                    {{-- <input name="contrato_id[]" {{ ($contratoVinculado > 0 || count($contrato->reservacion) !== 0) ? 'checked' : '' }}   class="contrato_id"  data-validar="{{ (count($contrato->reservacion) !== 0) ? 'true' : 'false' }}"  {{ ($contratoVinculado != 0 || count($contrato->reservacion) !== 0) ? 'disabled' : '' }}   type="checkbox" id="folio_asociado{{ $contrato->id }}" value="{{ $contrato->id }}" data-reservacion_id="{{ $reservacion->id }}">
                                    </input> --}}
            ...
            <div class="col-md-6">
                ...
                <div class="row" id="rowHabitacion1">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input class="form-control" id="noches" name="noches[]" type="text" value="{{ $reservacion->estancia->noches }}">
                        </input>
                        <span class="text-danger error-noches errors">
                        </span>
                    </div>
